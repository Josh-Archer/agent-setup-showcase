...
def omp_models(
    ...
    if matrix and matrix.get("defaults", {}).get("codex") == "gpt-6-astra":
        cfg = matrix.get("roles", {}).get(role, {})
        effort = cfg.get("reasoning_effort", "medium")
        model = cfg.get("codex") or matrix["defaults"]["codex"]
        return [f"openai-codex/{model}:{effort}"]
...
def write_surfaces(root: Path, *, prune: bool = False) -> tuple[int, list[Path]]:
    ...
    for source in agents:
        ...
        if name in {"lead", "investigator", "reviewer", "validation-runner"}:
            native = {
                "name": name, "description": value(meta, "description", name),
                "model": model, "model_reasoning_effort": reasoning,
                "developer_instructions": body,
            }
            (source_dir / f"{name}.toml").write_text(
                "# Generated by scripts/sync_agent_surfaces.py; edit the Markdown source.\n"
                + "".join(f"{key} = {json.dumps(val)}\n" for key, val in native.items()),
                encoding="utf-8",
            )
...
def check_surfaces() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="agent-surface-check-") as tmp:
        ...
        shutil.copytree(SOURCE_DIR, dest_source, ignore=shutil.ignore_patterns("*.toml"))
        ...
        mismatches: list[str] = []
        for native in sorted((tmp_root / ".codex" / "agents").glob("*.toml")):
            actual = ROOT / ".codex" / "agents" / native.name
            if not actual.exists() or actual.read_bytes() != native.read_bytes():
                mismatches.append(f"native Codex agent differs: {native.name}")
